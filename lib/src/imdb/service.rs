...
impl Service {
  ...
  pub fn new(cache_dir: &Path, force_db_update: bool, progress_fn: impl Fn(Option<u64>, u64)) -> Res<Self> {
    ...
    let movies_data = io_file::read_into_static(&movies_db_filename)?;
    let series_data = io_file::read_into_static(&series_db_filename)?;
  ...
  fn ensure_db_files(
    ...
    let needs_update = {
      force_db_update
        ...
        || io_file::older_than(&io_file::exists(series_db_filename)?, max_age)
    };
