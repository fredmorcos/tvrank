...
use tvrank::imdb::{Imdb, ImdbErr, ImdbQuery, ImdbTitleId, ImdbTitleIdErr};
...
enum Err {
  ...
  SearchString(#[from] SearchStringErr),
  #[error("Output error: {0}")]
  Print(#[from] print::Err),
  #[error("Directory error: {0}")]
  Walkdir(#[from] walkdir::Error),
  #[error("IMDB title ID error: {0}")]
  ImdbTitleId(#[from] ImdbTitleIdErr),
...
fn create_keywords_set(title: &str) -> Result<Vec<SearchString>, Err> {
...
fn imdb_title<'a>(
  ...
  search_opts: &SearchOpts,
  exact: bool,
  printer: Box<dyn Printer<Error = crate::print::Err>>,
) -> Result<(), Err> {
...
fn imdb_mark(dir: &Path, id: &str, imdb: &Imdb, force: bool) -> Result<(), Err> {
  ...
  if !dir.is_dir() {
    return Err(Err::NotDir(dir.to_owned()));
  }
  ...
  if imdb
    ...
    .is_none()
  {
    return Err(Err::UnknownImdbId(id.to_owned()));
  }
...
fn create_project() -> Result<ProjectDirs, Err> {
...
fn create_cache_dir(project: &ProjectDirs) -> Result<&Path, Err> {
...
fn get_imdb_url() -> Result<Url, Err> {
...
fn create_output_printer(
  ...
  general_opts: &GeneralOpts,
) -> Box<dyn Printer<Error = crate::print::Err>> {
  match output_format {
...
fn create_imdb_service(app_cache_dir: &Path, force_update: bool) -> Result<Imdb, Err> {
